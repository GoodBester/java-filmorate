...
public class UserDbStorage implements UserStorage {
    ...
    public List<User> getUsers() {
        List<User> u;
        String select = "SELECT u.id, u.name, u.email, u.login, u.birthday, f.friend, f.STATUS FROM USER u ";
        String joinFriends = "LEFT JOIN FRIENDS f ON u.ID = f.USER ";
        String condition = "ORDER BY u.id;";
        try {
            u = jdbcTemplate.queryForObject(select + joinFriends + condition, listUserRowMapper());
    ...
    public User getUser(int id) throws NotFoundException {
        ...
        String select = "SELECT u.id, u.name, u.email, u.login, u.birthday, f.friend, f.STATUS FROM USER u ";
        String joinFriends = "left JOIN FRIENDS f ON u.ID = f.USER ";
        String condition = "WHERE u.id = ?;";
        try {
            u = jdbcTemplate.queryForObject(select + joinFriends + condition, userRowMapper(), id);
        } catch (EmptyResultDataAccessException e) {
    ...
    public List<User> getFriends(int id) throws NotFoundException {
        ...
        List<User> u;
        String select = "SELECT u.id, u.name, u.email, u.login, u.birthday, f.friend, f.STATUS FROM USER u ";
        String joinFriends = "LEFT JOIN FRIENDS f ON u.ID = f.friend ";
        String condition = "WHERE f.user=? ORDER BY u.id;";
        try {
            u = jdbcTemplate.queryForObject(select + joinFriends + condition, listUserRowMapper(), user.getId());
        } catch (EmptyResultDataAccessException e) {
            return List.of();
        }
        return u;
    ...
    @Override
    public List<User> getCommonFriends(int id, int friendId) throws NotFoundException {
        User user = getUser(id);
        User friend = getUser(friendId);
        List<User> u;
        String select = "SELECT u.id, u.name, u.email, u.login, u.birthday, f.friend, f.STATUS FROM USER u ";
